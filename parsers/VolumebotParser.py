        ...
        for item in items:
            try:
                ...
                try:
                    ...
                    logger.warning(f"Failed to fetch DexScreener data for {tokenId}: {str(e)}")
                    priceData = None
                ...
                volumeToken = VolumeToken(
                    ...
                    price=Decimal(str(priceData.price)) if priceData else _parseDecimal(item['price_1h_raw']),
                    marketcap=Decimal(str(priceData.marketCap)) if priceData else _parseDecimal(item['marketCap_raw']),
                ...
                logger.info(f"Successfully parsed token {tokenId} - {item['token_name']} with DexScreener data: {bool(priceData)}")
