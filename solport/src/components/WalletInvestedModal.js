...
import SmartMoneyMovementsModal from './SmartMoneyMovementsModal';
...
function WalletInvestedModal({ token, onClose }) {
  ...
  const [processingMovements, setProcessingMovements] = useState(false);
  const [movementsStatus, setMovementsStatus] = useState(null);
  const [selectedWalletForMovements, setSelectedWalletForMovements] = useState(null);
  ...
  const processTokenMovements = async () => {
    if (!token || !token.tokenid) return;
    
    setProcessingMovements(true);
    setMovementsStatus(null);
    
    try {
      const response = await axios.post(
        'http://localhost:8080/api/smartmoneymovements/process/token',
        { token_address: token.tokenid },
        {
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
      ...
      setMovementsStatus({
        success: true,
        message: response.data.message || 'Successfully processed token movements'
      });
    } catch (err) {
      console.error('Error processing token movements:', err);
      setMovementsStatus({
        success: false,
        message: err.response?.data?.message || 'Failed to process token movements'
      });
    } finally {
      setProcessingMovements(false);
      // Clear status after 3 seconds
      setTimeout(() => setMovementsStatus(null), 3000);
    }
  };
  ...
  // Handle View Movements button click
  const handleViewMovements = (wallet, e) => {
    // Stop propagation to prevent row click
    e.stopPropagation();
    setSelectedWalletForMovements(wallet);
  };

  // Close smart money movements modal
  const closeMovementsModal = () => {
    setSelectedWalletForMovements(null);
  };
  ...
  return (
    <div className="wallet-modal-backdrop" onClick={onClose}>
      <div className="wallet-modal-content" ref={modalRef} onClick={handleModalClick}>
        <div className="wallet-modal-header">
          ...
          <div className="token-id-container">
            ...
            <button 
              className={`process-movements-button ${processingMovements ? 'processing' : ''}`}
              onClick={processTokenMovements}
              disabled={processingMovements}
              title="Process smart money movements for this token"
            >
              <FaBrain className="button-icon" />
              {processingMovements ? 'Processing...' : 'Process Movements'}
              {processingMovements && <div className="button-spinner"></div>}
            </button>
            {movementsStatus && (
              <div className={`movements-status ${movementsStatus.success ? 'success' : 'error'}`}>
                {movementsStatus.message}
              </div>
            )}
          </div>
                          ...
                          <td className="wallet-address">
                            <div className="address-container">
                              ...
                              <button 
                                ...
                                onClick={(e) => handleViewMovements(wallet, e)}
                                title="View Smart Money Movements"
                              >
                                <FaChartLine />
                              </button>
                            </div>
      ...
      {selectedWalletForMovements && (
        <SmartMoneyMovementsModal
          wallet={selectedWalletForMovements}
