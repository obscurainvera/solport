...
function Operations() {
  ...
  const [movementsWalletAddress, setMovementsWalletAddress] = useState('');
  ...
  const processSmartMoneyMovements = () => {
    if (!movementsWalletAddress) {
      showStatus('smart-money-movements-status', 'Please enter a wallet address', true);
      return;
    }

    showLoading('process-movements');
    console.log(`Sending XHR request to: ${API_BASE_URL}/api/smartmoneymovements/process/wallet`);
    
    const xhr = new XMLHttpRequest();
    xhr.open('POST', `${API_BASE_URL}/api/smartmoneymovements/process/wallet`, true);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.setRequestHeader('Accept', 'application/json');
    ...
    xhr.onload = function() {
      ...
      if (xhr.status >= 200 && xhr.status < 300) {
        try {
          const data = JSON.parse(xhr.responseText);
          console.log('XHR parsed data:', data);
          showStatus('smart-money-movements-status', data.message || 'Successfully processed smart money movements');
        } catch (e) {
          console.error('Error parsing response:', e);
          showStatus('smart-money-movements-status', 'Error parsing response', true);
        }
      } else {
        showStatus('smart-money-movements-status', `Error: ${xhr.status} ${xhr.statusText}`, true);
      }
      hideLoading('process-movements');
    };
    
    xhr.onerror = function() {
      console.error('XHR error occurred');
      showStatus('smart-money-movements-status', 'Network error occurred', true);
      hideLoading('process-movements');
    };
    
    xhr.send(JSON.stringify({
      wallet_address: movementsWalletAddress
    }));
  };
            ...
            <div className="section-row">
              ...
              <div className="col">
                <div className="luxury-card">
                  <div className="card-content">
                    <div className="card-header">
                      <h3>Smart Money Movements</h3>
                      <div className="badge badge-gold">Analytics</div>
                    </div>
                    <div className="pattern pattern-grid"></div>
                    <p>
                      Process and analyze smart money movements for a specific wallet.
                    </p>
                    <div className="form-group">
                      ...
                      <input 
                        type="text" 
                        className="luxury-input" 
                        value={movementsWalletAddress} 
                        onChange={(e) => setMovementsWalletAddress(e.target.value)}
                        placeholder="Enter wallet address"
                      />
                    </div>
                    <button 
                      className="luxury-button" 
                      onClick={processSmartMoneyMovements}
                      disabled={loading['process-movements']}
                    >
                      Process Movements
                      {loading['process-movements'] && <div className="loading-spinner"></div>}
                    </button>
                    {statusMessages['smart-money-movements-status']?.visible && (
                      <div className={`status-message ${statusMessages['smart-money-movements-status']?.isError ? 'error' : ''}`}>
                        {statusMessages['smart-money-movements-status']?.message}
                      </div>
                    )}
                  </div>
                </div>
