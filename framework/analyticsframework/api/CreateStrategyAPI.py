...
class CreateStrategyAPI:
    ...
    def addStrategy(self, strategyData: Dict[str, Any]) -> Optional[int]:
        ...
        try:
            ...
            strategyConfig = {
                ...
                'strategyentryconditions': self._serializeEntryConditions(strategyData['entry_conditions']),
                'chartconditions': self._serializeChartConditions(strategyData.get('chart_conditions')),
                'investmentinstructions': self._serializeInvestmentRules(strategyData['investment_instructions']),
                'profittakinginstructions': self._serializeProfitInstructions(strategyData['profit_taking_instructions']),
            ...
            # Log final config
            logger.info(f"Prepared strategy config: {json.dumps(strategyConfig, indent=2)}")
    ...
    def updateStrategy(self, strategyId: int, 
                       updateData: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Handle all configuration components consistently
            configMappings = {
                'entry_conditions': ('strategyentryconditions', self._serializeEntryConditions),
                'chart_conditions': ('chartconditions', self._serializeChartConditions),
                'investment_instructions': ('investmentinstructions', self._serializeInvestmentRules),
                'profit_taking_instructions': ('profittakinginstructions', self._serializeProfitInstructions),
                'risk_management_instructions': ('riskmanagementinstructions', self._serializeRiskInstructions),
                'moon_bag_instructions': ('moonbaginstructions', self._serializeMoonBagRules)
            }

            # Process each configuration component
            for inputKey, (dbKey, serializer) in configMappings.items():
                if inputKey in updateData:
                    updateConfig[dbKey] = serializer(updateData[inputKey])
            ...
            for inputKey, dbKey in basicFieldMappings.items():
                if inputKey in updateData:
                    updateConfig[dbKey] = updateData[inputKey]

            if not updateConfig:
                raise ValueError("No valid fields to update")
                
            # Always update the updated_time
            updateConfig['updatedtime'] = datetime.now()

            # Perform update
            success = self.analyticsHandler.updateStrategy(strategyId, updateConfig)
            if not success:
                raise ValueError(f"Failed to update strategy {strategyId}")

            return True

        except ValueError as e:
            logger.error(f"Validation error updating strategy: {str(e)}")
            raise
        except Exception as e:
            logger.error(f"Error updating strategy: {str(e)}")
            raise ValueError(f"Error updating strategy: {str(e)}")

    def activateStrategy(self, strategyId: int) -> bool:
        """Activate a strategy"""
        try:
            updateConfig = {
                'active': True,
            ...
            success = self.analyticsHandler.updateStrategy(strategyId, updateConfig)
            if not success:
                raise ValueError(f"Failed to activate strategy {strategyId}")
            return True
        except Exception as e:
            logger.error(f"Error activating strategy: {str(e)}")
            raise ValueError(f"Error activating strategy: {str(e)}")

    def deactivateStrategy(self, strategyId: int) -> bool:
        ...
        try:
            ...
            success = self.analyticsHandler.updateStrategy(strategyId, updateConfig)
            if not success:
                raise ValueError(f"Failed to deactivate strategy {strategyId}")
            return True
        except Exception as e:
            logger.error(f"Error deactivating strategy: {str(e)}")
            raise ValueError(f"Error deactivating strategy: {str(e)}")
        ...
        try:
            ...
            for field in requiredFields:
                if field not in data:
                    raise ValueError(f"Missing required field in entry conditions: {field}")
    ...
    def _serializeInvestmentRules(self, data: Dict) -> str:
        ...
        try:
            ...
            for field in requiredFields:
                if field not in data:
                    raise ValueError(f"Missing required field in investment instructions: {field}")
                    
            # Convert to database column names
            investmentRules = {
                'entrytype': data.get('entry_type', 'BULK'),
                'allocatedamount': str(data.get('allocated_amount', '0')),
                'maxpositionsize': str(data.get('max_position_size')) if data.get('max_position_size') else None,
                'maxportfolioallocation': str(data.get('max_portfolio_allocation')) if data.get('max_portfolio_allocation') else None,
                'maxtokenallocation': str(data.get('max_token_allocation')) if data.get('max_token_allocation') else None
            }
            ...
            if 'dca_rules' in data and data['dca_rules']:
                ...
                investmentRules['dcarules'] = {
                    ...
                    'pricedeviationlimit': str(dca_data.get('price_deviation_limit_pct')) if dca_data.get('price_deviation_limit_pct') else None
                }
        ...
        try:
            ...
            for target in data:
                if 'price_target_pct' not in target or 'sell_amount_pct' not in target:
                    raise ValueError("Each profit target must have price_target_pct and sell_amount_pct")
    ...
    def _serializeRiskInstructions(self, data: Dict) -> str:
        ...
        try:
            ...
            for field in requiredFields:
                if field not in data:
                    raise ValueError(f"Missing required field in risk management: {field}")
    ...
    def _serializeMoonBagRules(self, data: Optional[Dict]) -> Optional[str]:
        ...
        try:
            ...
            for field in requiredFields:
                if field not in data:
                    raise ValueError(f"Missing required field in moon bag instructions: {field}")
